...
class Window(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        export_grid.addWidget(self.export_btn, 2, 0, 1, 2)
    ...
    def export(self):
        ...
        for prediction, name in export_classes.items():
            ...
            if self.export_regions_btn:
                print("Exporting pick regions.")
                pick_centers.clear()

                for pick in n_new_groups:
                    pick_locs = filtered_locs[filtered_locs["group"] == pick]
                    pick_centers.append([float(np.mean(pick_locs.x)), float(np.mean(pick_locs.y))])
